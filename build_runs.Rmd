...
sample_window_example <- sample_window('04/07/2023', 10, our_gcm)
```
...
```{r}
filter_df_year <- function(df, climate_table, start_date) {
  # df$water_year <- year(if_else(month(df$time) >= 10, df$time + months(3), df$time - months(9)))
...
filter_df_season <- function(df, climate_table) {
  #df$water_year <- year(if_else(month(df$time) >= 10, df$time + months(3), df$time - months(9)))
  df <- df %>% group_by(year = lubridate::year(time), season) %>% summarise(wind = mean(wind),
                                                                    max_temp = mean(max_temp),
                                                                    min_temp = mean(min_temp),
                                                                    precip = sum(precip),
                                                                    min_humidity = mean(min_humidity),
 ...
 for (j in 1:nrow(climate_table)) {
    ...
    df2 <- df %>% filter(season == season[j])
     
    lower_perc <- quantile(df2[[climate_var]], lower, na.rm = TRUE)

    upper_perc <- quantile(df2[[climate_var]], upper, na.rm = TRUE)

    df3 <- df2 %>% filter(!is.na(!!sym(climate_var)), !!sym(climate_var) >= lower_perc, !!sym(climate_var) <= upper_perc)
 }
season_year <- paste(df3$season, df3$year)
return(season_year)
}

test <- filter_df_season(season, climate_vars)

```

```{r}
season_def <- list(wet_season = c(11, 12, 1, 2, 3, 4), dry_season = c(5, 6, 7, 8, 9, 10))
...
season <- add_season(sample_window_example)
```
...
filter_season <- function(rcp,
  ...
  # Call filter_df at index i of the climate variables list
  season_year <- filter_df_season(df, climate_table[[i]])

    # Appends the output to the df_list
    df_list[[i]] <- season_year
    print(df_list)
    start_date <- start_date + years(1) # Add one year to start_date
    season_month <- month(start_date) + lubridate::month(6)
}
...
#df <- sample_window('10/07/2023', 10, our_gcm)
```
