...
r_files <- list.files(pattern = "\\.R$", full.names = TRUE) #for mal lol 
...
no_matches <- lapply(run_samples, function(x) length(x))

# Create a dataframe with the index and the count of years/seasons
no_matches <- data.frame(Count = unlist(no_matches))
...
```{r}
find_all_grid_cells()
get_all_grid_cells(df_names = ui_df_names) # we need to make a root folder for all the grid cells probably in erica's function? for ALL grid cells, this needs to be one step up above in the loop function
```
