...
```{r}
MIROC5_45_grid_1 <- MIROC5_45_grid_1 %>% mutate(water_year = ifelse(month(time) > 9, year(time) + 1, year(time))) # vc modified

MIROC5_45_grid_2 <- MIROC5_45_grid_2 %>% mutate(water_year = ifelse(month(time) > 9, year(time) + 1, year(time))) # vc added
```
