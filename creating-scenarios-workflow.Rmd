...
```{r}
MIROC5_45_grid_1 <- read.csv(url("https://raw.githubusercontent.com/fire-futures/climate-scenarios/main/MIROC5_rcp45_1")) # vc modified

MIROC5_45_grid_2 <- read.csv(url("https://raw.githubusercontent.com/fire-futures/climate-scenarios/main/MIROC_rcp45_2")) #sourcing example cleaned data # vc modified

#setwd("/Users/mallorygiesie/github/climate-scenarios/build_functions") # vc commented out

#setwd(".../climate-scenarios/build_functions") # vc commented out
...
```{r}
MIROC5_45_grid_1 <- MIROC5_45_grid_1 %>% 
  mutate(water_year = ifelse(month(time) > 9, year(time) + 1, year(time))) %>%  # vc modified
  mutate(season = ifelse(lubridate::month(time) %in%  c(11, 12, 1, 2, 3, 4), "wet", "dry")) # Erica modified to resolve season column

MIROC5_45_grid_2 <- MIROC5_45_grid_2 %>% 
  mutate(water_year = ifelse(month(time) > 9, year(time) + 1, year(time))) %>%  # vc modified
  mutate(season = ifelse(lubridate::month(time) %in%  c(11, 12, 1, 2, 3, 4), "wet", "dry")) # Erica modified to resolve season column
```
...
# run these even though we've loaded in data
...
#specify data frame in ui_base_data 
ui_base_data <- MIROC5_45_grid_2
...
run_samples <<-
  find_matches(
    base_data = ui_base_data, # vc modified
